...
const (
	...
	readNotesByTime  string = `SELECT name, description, start_date as start_time, end_date as end_time FROM calen_go.notes WHERE start_date >= CAST(? AS DATE) AND end_date <= CAST(? AS DATE);`
)
