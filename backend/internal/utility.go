...
const (
	...
	readConflictedNotes string = `SELECT T1.name, COALESCE(T1.description, ''), T1.start_date, T1.end_date, T2.name, COALESCE(T2.description, ''), T2.start_date, T2.end_date FROM calen_go.notes T1 JOIN calen_go.notes T2 ON T1.id != T2.id AND (T1.start_date >= T2.start_date AND T1.start_date <= T2.end_date OR T1.end_date >= T2.start_date AND T1.end_date <= T2.end_date) WHERE T1.author_id = T2.author_id AND T1.author_id = ? ORDER BY T1.name, T2.name;`
)
